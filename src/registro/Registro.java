...
public class Registro {
    ...
    public void registrarse(byte[] template, long mhDB, int numCiclos, byte[][] plantillasPrerregistro) throws InterruptedException {
        ...
        int[] idDevuelto = new int[1];
        /*<- el id se almacena en este array*/
        int[] score = new int[1];
        ...
        if (numCiclos > 0 && FingerprintSensorEx.DBMatch(mhDB, plantillasPrerregistro[numCiclos - 1]/*Template 1, es 
                        la última huella que ingresamos antes de esta*/, /*Template 2*/ template/*template es la huella que acabamos de ingresar*/) <= 0) /*DBmatch compara 2 huellas diferentes*/ {
        ...
        if (numCiclos == 2) {//Si enroll_idx es igual a 3, significa que se ha registrado el dedo 3 veces
            int[] longitudDevuelta = new int[1];
            longitudDevuelta[0] = 2048;
            /*<- Longitud de la plantilla registrada*/
            byte[] plantillaRegistrada = new byte[longitudDevuelta[0]];//regTemp es plantilla registrada

            if (0 == (ret = FingerprintSensorEx.DBMerge/*Esta función se usa para combinar plantillas de huellas dactilares registradas*/(mhDB/*catch handle*/, plantillasPrerregistro[0]/*Plantilla preregistrada1*/,
                    ...
                    && 0 == (ret = FingerprintSensorEx.DBAdd/*Esta función se utiliza para agregar una plantilla registrada a la memoria.*/(mhDB, 1/*ID de la huella digital*/, plantillaRegistrada/*Plantilla registrada*/))) {
    ...
    public boolean registroBD(String ruta, String name, long mhDB) throws InterruptedException, SQLException, ClassNotFoundException {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de registrar usuario:\n " + e);
            registr = false;
        } finally {
            try {
                pru.closeCon();
